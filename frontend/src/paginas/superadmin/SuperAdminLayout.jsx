...
export default function SuperAdminLayout() {
  ...
  return (
    <div className="min-h-screen w-full flex bg-[#06060a] dark:bg-[#06060a] selection:bg-violet-500 selection:text-white">
      <ClickSpark sparkColor='#7c3aed' sparkSize={10} sparkRadius={16} sparkCount={6} duration={350}>
        <div className="flex w-full min-h-screen relative overflow-hidden">
          ...
          {/* ── Sidebar ── */}
          <aside className={`fixed inset-y-0 left-0 z-40 w-64 bg-[#0d0d14] border-r border-white/5 flex flex-col justify-between transition-transform duration-300 lg:translate-x-0 lg:static ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
            ...
            <div>
              <div className="p-6 border-b border-white/5 flex items-center justify-between lg:justify-start relative">
                ...
                <button
                  ...
                  className="lg:hidden absolute right-4 p-1.5 rounded-lg hover:bg-white/5 text-neutral-400 hover:text-white transition-all"
                >
                  <X size={16} />
                </button>
              </div>
              ...
              {/* Etiqueta de entorno */}
              <div className="mx-4 mt-4 px-3 py-2 rounded-lg bg-violet-500/10 border border-violet-500/20 flex items-center gap-2">
                <Zap size={12} className="text-violet-400" />
                <span className="text-violet-300 text-[11px] font-medium">Panel de Control Global</span>
              </div>
                  ...
                  return (
                    <Link
                      ...
                      className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all duration-200 group ${
                        active
                          ? 'bg-gradient-to-r from-violet-600/30 to-indigo-600/20 text-white border border-violet-500/30 shadow-sm shadow-violet-900/20 translate-x-1'
                          : 'text-neutral-400 hover:text-white hover:bg-white/5 hover:translate-x-0.5'
                      }`}
                    >
                      <IconComp size={17} className={active ? 'text-violet-400' : 'text-neutral-500 group-hover:text-neutral-300'} />
                      <div className="flex-1">
                        <span>{item.name}</span>
                        <p className="text-[10px] text-neutral-500 font-normal leading-tight">{item.desc}</p>
                      </div>
                      {active && <ChevronRight size={14} className="text-violet-400 ml-auto" />}
                    </Link>
                  );
                })}
              </nav>
              ...
              {user && (
                <div className="flex items-center gap-3 px-3 py-2">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-violet-400 font-medium">Super Administrador</p>
                  </div>
                </div>
              )}
              <button
          ...
          <div className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
            ...
            {/* Header */}
            <header className="h-16 bg-[#0d0d14] border-b border-white/5 flex items-center justify-between px-6 z-20">
              <div className="flex items-center gap-4">
                <button
                  onClick={() => setSidebarOpen(true)}
                  className="lg:hidden p-2 -ml-2 rounded-xl hover:bg-white/5 text-neutral-400 hover:text-white transition-all"
                >
                  <Menu size={20} />
                </button>
              ...
              <div className="flex items-center gap-3">
                <ThemeToggle />
                {/* Badge de rol */}
                <div className="hidden sm:flex items-center gap-1.5 px-3 py-1 rounded-full bg-violet-500/15 border border-violet-500/25">
                  <Shield size={11} className="text-violet-400" />
                  <span className="text-[11px] font-semibold text-violet-300 uppercase tracking-wider">Super Admin</span>
                </div>
              </div>
            </header>

            {/* Contenido dinámico */}
            <main className="flex-1 overflow-y-auto bg-[#06060a] p-6 md:p-8">
