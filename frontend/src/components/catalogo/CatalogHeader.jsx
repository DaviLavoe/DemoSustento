    ...
    <>
      ...
      <section className="sticky top-20 bg-white/80 dark:bg-[#0a0a0a]/80 backdrop-blur-xl border-b border-[#e5e5e5]/50 dark:border-neutral-800 z-20 transition-colors duration-300">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center justify-start gap-2 overflow-x-auto pb-2 pt-1 md:py-1 scrollbar-none animate-reveal delay-400 w-[calc(100%+2rem)] -mx-4 px-4 md:w-auto md:flex-1 md:min-w-0 md:mx-4 snap-x snap-mandatory">
            <Filter size={16} className="text-neutral-400 hidden lg:inline shrink-0" />
            {categories.map((cat) => (
              <button
                key={cat}
                onClick={() => setSelectedCategory(cat)}
                className={`px-4 py-2.5 rounded-xl text-xs font-bold whitespace-nowrap transition-all duration-300 active:scale-95 shrink-0 snap-center ${
