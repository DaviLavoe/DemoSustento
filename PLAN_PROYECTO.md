...
- **Davi (Front):** Construir la UI del panel de administración (Dashboard): Tablas de inventario, y modales/formularios para crear, editar y eliminar productos.
- **Angelo (Back):** Crear endpoints CRUD completos para el inventario (`POST`, `PUT`, `DELETE` en `/api/productos`).
- **Javicho (Data):** Configurar Supabase *Storage* para el almacenamiento de imágenes de los productos. Apoyar a Davi en la integración de subida de archivos.

### Semana 3: Micro-CMS y Catálogo Público (Vitrina)
- **Davi (Front):** Crear la vista pública del catálogo (`/catalogo/:slug-empresa`). Mostrar la cuadrícula de productos con un diseño dinámico y atractivo.
- **Angelo (Back):** Crear endpoints públicos de solo lectura para alimentar el catálogo, optimizados para búsquedas y filtros rápidos.
- **Javicho (Data):** Crear la tabla `configuracion_catalogo` en Supabase (para guardar colores, logo, y mensaje de WhatsApp de cada empresa).
